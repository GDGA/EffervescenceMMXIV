...
public class Collage extends Activity implements OnClickListener, OnCheckedChangeListener {
	...
	public void onClick(View v) {
		...
		switch(v.getId()){
			case R.id.checkBox1:
				if(cb.isChecked()){
						try{
							Favourites entry = new Favourites(Collage.this);	
							entry.openandwrite();
							entry.createEntry("Collage", "com.example.effervescencemmxiv.COL");
							entry.close();
						}catch(Exception e){
							diditwork = false;
					...
					try{
						Favourites entry = new Favourites(Collage.this);	
						entry.openandwrite();
						entry.deleteTitleGivenName("Collage");
						entry.close();
					}catch(Exception e){
						String error = e.toString();
						Toast.makeText(getApplicationContext(), error, Toast.LENGTH_SHORT).show();
		...
		case R.id.imageView1:
			...
			it.putExtra("address", "com.example.effervescencemmxiv.COL");
			startActivity(it);
			break;
